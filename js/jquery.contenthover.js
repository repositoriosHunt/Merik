/*!
 * contenidoHover jQuery plugin v0.1
 * http://www.backslash.gr/demos/contenidoHover-jquery-plugin/
 *
...
(function($){  
	var methods = {
		init : function(options) {
			var defaults = {  
				data_selector: '.contenidoHover',	// The selector for the element that will be the content of the overlay element to show on hover 
		...
		destroy : function() {
			return this.each(function(){
				$(this).show();
				$('.ch_element').remove();
				$(window).unbind('.contenidoHover');
			});
		}
	};
	
	$.fn.contenidoHover = function(method) {  
		if (methods[method]) {
			return methods[method].apply(this, Array.prototype.slice.call(arguments, 1));
		} else if (typeof method === 'object' || !method) {
			return methods.init.apply(this, arguments);
		} else {
			$.error('Method '+ method +' does not exist in contenidoHover plugin.');
		}  
	}; 
